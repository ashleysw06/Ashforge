class PlayerController extends Component {
    ...
    update(deltaTime) {
        ...
        // this.transform.scene.game.engine.screenX = (this.transform.vector2D.position.x + game.engine.worldSpaceMouseX) / 2;
        // this.transform.scene.game.engine.screenY = (this.transform.vector2D.position.y + game.engine.worldSpaceMouseY) / 2;

        const xDistance = this.transform.vector2D.position.x - game.engine.worldSpaceMouseX;
        const yDistance = this.transform.vector2D.position.y - game.engine.worldSpaceMouseY;
        const angle = Math.atan2(yDistance, xDistance) * (180 / Math.PI);
        this.transform.rotation2D.setRotDeg(angle);

        // GAME CAMERA FOLLOW PLAYER
        const distance = Math.sqrt(Math.pow(xDistance, 2) + Math.pow(yDistance, 2))
        this.transform.scene.game.engine.screenX = (this.transform.vector2D.position.x - xDistance / Math.sqrt(Math.max(100, distance * 0.2)));
        this.transform.scene.game.engine.screenY = (this.transform.vector2D.position.y - yDistance / Math.sqrt(Math.max(100, distance * 0.2)));
